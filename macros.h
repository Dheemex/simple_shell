...
/* the Resume from the unused attribute */
#define UNUSED __attribute__((unused))

/* the buffer size for each read call in _getline */
...
"cd:\tcd [dir]\n\n"\
"	it Change the shell working directory.\n\n"\
"	If no argument is given to cd the command be interpreted\n"\
"	as cd $HOME.\n"\
"	if the argumenthelp is - (cd -), the command be interpreted\n"\
"	as cd $OLDPWD.\n\n"

#define HELP_EXIT_MSG "exit=\n"\
"exit:\texit [STATUS]\n\n"\
"	Exit of the simple-shell.\n\n"\
"	Exits the shell a status of N.  If N is omitted, the exit status\n"\
